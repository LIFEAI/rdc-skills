...
1. **Gather inputs:**
   - Research doc from preplan (if exists): `docs/research/<topic>.md`
...
4. **Define work packages** — break into agent-dispatchable units:
   ...
   - Assign an agent type to each work package from the typed dispatch table in rdc:build. Include the guide file path (from `docs/guides/`) in each work package description.
   - Estimate: small (1 agent, <500 LOC), medium (1 agent, 500-1500 LOC), large (needs splitting)

5. **Write plan doc** to `docs/plans/<topic-slug>.md`:
   ```markdown
   # Plan: <Topic>
...
- Plan doc goes in `docs/plans/`, not `.planning/`
...
- Reference the relevant guide file from `docs/guides/` for agent context
